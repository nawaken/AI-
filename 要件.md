...
- **CM-01 Confluence連携**: 本要件書（`REQ-EL-001`）をConfluenceページに添付し、ページ見出しに図ID（DF-01、DF-02等）と要件ID（FR-01等）のアンカーリンクを設ける。各Mermaid図はConfluenceのMermaidプラグインで再現し、IDを見出しに記載することで双方向参照を可能にする。
- **CM-02 変更申請フロー**: 要件・図の変更はConfluenceページ上で「変更提案」セクションを設け、変更理由・対象ID・影響範囲・承認者を明記してからレビュー依頼を行う。承認後にGit上の`要件.md`とConfluenceを同時更新する。
- **CM-03 バージョニング**: `REQ-EL-001` のリビジョン番号（例: `REQ-EL-001 v1.1`）をConfluenceとMarkdownの双方で更新し、履歴表にコミットIDと変更概要を記録する。
- **CM-04 参照整合性チェック**: 要件IDや図IDを追加/削除する際は、Confluenceの検索とリポジトリ内grepで未更新リンクがないか確認し、破壊的変更の場合は互換ガイドラインを追記する。
